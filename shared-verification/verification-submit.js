// ═══════════════════════════════════════════════════════════════════════
// 框架层（FRAMEWORK LAYER）— S3 verification
//
// 文件定位：shared-verification/verification-submit.js
//
// 边界说明（BOUNDARY）：
// ✅ 负责 —— 验证码提交阶段调度器。
...
// 依赖方：Dreamina/0.0.3/Sn-xxx/ 下对应的 adapter（调用 verification-adapter（当前绑定 Dreamina），拉取并提交邮件验证码并返回 VerificationResult）
// ═══════════════════════════════════════════════════════════════════════
'use strict';
...
// ==============================
// 基础工具层
// 负责 adapter contract 解析与 shared result 归一化。
// ==============================
...
// 负责按固定顺序执行 ready -> fetch -> resolve -> fill -> confirm，
// 并在 shared 层承接有限 verification retry policy。
// ==============================
...
async function runVerificationSubmitStage(options = {}) {
  ...
  // ==============================
  // ready gate
  // 只有确认当前页面已进入 verification 阶段，后续 fetch / fill / confirm 才有意义。
  // ==============================
  ...
  verificationPhaseTrace.readyMs = readyTimer.elapsedMs();
  ...
  if (!verificationReady?.ok) {
    ...
    return normalizeVerificationStageResult({
      ...
      detail: {
        ...
        verificationReady,
        classified,
        verificationPhaseTrace,
      },
    });
  }

  // 从 runtime 中读取 verification 阶段内部重试上限；默认允许第一次尝试 + 1 次重试。
  // 这是 shared 层唯一允许的 verification retry 主循环边界，后续不应继续叠加第二层重试框架。
  ...
  // shared 层允许有限重试，但严格受 verificationRetryMaxAttempts 限制。
  // 这里只允许一层 attempt loop，不允许在 adapter 内再扩成递归/嵌套 orchestration。
  // ==============================
  ...
  logStageProgress('verification-submit', '拉取验证码', {
    ...
    // ------------------------------
    // Step 3: resolve verification input
    // ------------------------------
  ...
  logStageProgress('verification-submit', '定位验证码输入框', {
    ...
    attemptTrace.resolveInputMs = resolveTimer.elapsedMs();
  ...
  logStageProgress('verification-submit', '确认验证码提交结果', {
    ...
    attemptTrace.confirmSubmitMs = confirmTimer.elapsedMs();
    attemptTrace.confirmState = String(confirmResult?.state || '');
    ...
    // ------------------------------
    // retry policy
    // 只有明确 wrong-code / rate-limited 等可恢复信号时，shared 层才允许 resend 后进入下一轮。
    // usedCodes 只在“已确认当前验证码不可再试”时写入，避免把未验证失败的 code 过早拉黑。
    // ------------------------------
