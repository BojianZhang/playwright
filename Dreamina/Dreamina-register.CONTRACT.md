...
## post-auth-ready
- ??????????`POST_AUTH_READY_ONLY`
- ???????? birthday/profile-completion ???????????????? session/workspace ????
- ???????`account-delivery`
- ?????? birthday title / subtitle / Year / Month / Day / Next ???? `REGISTRATION_COMPLETE`
- ????? session + UI ???????????? `REGISTRATION_COMPLETE`
