...
//
// 边界说明（BOUNDARY）：
// ✅ 负责 —— 通过 Firstmail API 拉取验证码并填入 Dreamina 验证码输入框。
// ✅ 负责 —— Dreamina 特有的 CSS 选择器、文案匹配、交互序列定义。
// ✅ 负责 —— 从 profiles/ 加载当前阶段的 Dreamina 配置（profile JSON）。
// ❌ 不负责 —— 阶段调度、重试策略、日志格式化（由框架层 shared-verification/stages/verification-submit.js 负责）。
// ❌ 不负责 —— 跨阶段状态传递（由 Dreamina-register.js 主链持有并传入 options）。
// ❌ 不负责 —— 任何非 Dreamina 平台的逻辑（Platform-specific, not reusable）。
//
// 被调用方：shared-verification/stages/verification-submit.js（框架层通过 options.verification-adapter 或直接调用注入）
// profiles：Dreamina/0.0.3/S3-verification/profiles/
// ═══════════════════════════════════════════════════════════════════════
'use strict';
...
const { waitForDreaminaCodeViaApi } = require('../../../shared-utils/firstmail-api');
...
const _verificationProfileCacheRef = { value: null };

function loadDreaminaVerificationProfile(options = {}) {
  return loadJsonProfileWithCache(DREAMINA_VERIFICATION_PROFILE_PATH, _verificationProfileCacheRef, options);
}
...
async function waitForDreaminaVerificationStageReady(page, runtime = {}, context = {}) {
  ...
  const steps = buildStepWaitList(0, Number(runtime?.verificationReadyPrimaryWaitMs || 300), Number(runtime?.verificationReadySecondaryWaitMs || 900));
