// ═══════════════════════════════════════════════════════════════════════
// 运行内容层（RUNTIME CONTENT LAYER）— S6 Dreamina
//
// 文件定位：Dreamina/0.0.3/S6-account-delivery/account-delivery-adapter.js
// 平台绑定：Dreamina（仅服务于 Dreamina 注册流程，非通用 adapter）
//
// 边界说明（BOUNDARY）：
// ✅ 负责 —— 将注册成功的 Dreamina 账号序列化写入交付产物（文件 / 回调）。
// ✅ 负责 —— Dreamina 特有的 CSS 选择器、文案匹配、交互序列定义。
// ✅ 负责 —— 从 profiles/ 加载当前阶段的 Dreamina 配置（profile JSON）。
// ❌ 不负责 —— 阶段调度、重试策略、日志格式化（由框架层 shared-account-delivery/stages/account-delivery.js 负责）。
// ❌ 不负责 —— 跨阶段状态传递（由 Dreamina-register.js 主链持有并传入 options）。
// ❌ 不负责 —— 任何非 Dreamina 平台的逻辑（Platform-specific, not reusable）。
//
// 被调用方：shared-account-delivery/stages/account-delivery.js（框架层通过 options.account-delivery-adapter 或直接调用注入）
// profiles：Dreamina/0.0.3/S6-account-delivery/profiles/
// ═══════════════════════════════════════════════════════════════════════
'use strict';
