...
const { createMutex } = require('../lib/batch-orchestration/mutex');
const { envInt, envFloat } = require('./env-tunables');

// 默认指向 data/card-pool.json;DREAMINA_CARD_POOL_FILE 可覆盖(仅供单测指向临时卡池,不污染生产数据)。默认行为不变。
const POOL_FILE = process.env.DREAMINA_CARD_POOL_FILE || path.join(__dirname, '..', 'data', 'card-pool.json');
