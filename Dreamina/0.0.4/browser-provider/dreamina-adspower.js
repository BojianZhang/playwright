...
const DEFAULT_API = process.env.DREAMINA_ADSPOWER_API || 'http://local.adspower.net:50325';
// AdsPower 鉴权令牌(本机网关一般无需 → 默认空,不加头;web 经 engine-runner 注入 DREAMINA_ADSPOWER_TOKEN)。
// 头名/前缀可覆盖以适配不同远程网关(默认 Authorization: Bearer <token>;前缀设空=发裸 token)。
const ADS_TOKEN = (process.env.DREAMINA_ADSPOWER_TOKEN || '').trim();
const ADS_AUTH_HEADER = process.env.DREAMINA_ADSPOWER_AUTH_HEADER || 'Authorization';
const ADS_AUTH_PREFIX = process.env.DREAMINA_ADSPOWER_AUTH_PREFIX != null ? process.env.DREAMINA_ADSPOWER_AUTH_PREFIX : 'Bearer ';
...
async function createAdsPowerRuntime(userId, opts = {}) {
  ...
  const identityRuntime = await applyConnectedRuntimeIdentity(context, page, {
    ...opts,
    ipSource: 'adspower',
  }).catch((error) => buildConnectedRuntimeIdentityError(error, 'adspower-context-identity-error'));
