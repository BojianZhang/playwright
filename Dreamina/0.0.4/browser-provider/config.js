...
//   env 命名：DREAMINA_<NAME>_API / DREAMINA_<NAME>_TOKEN，如 DREAMINA_BITBROWSER_API。
// ═══════════════════════════════════════════════════════════════════════
...
function providerConfig(name) {
  ...
  const env = {};
  if (process.env[`DREAMINA_${U}_API`]) env.apiBase = process.env[`DREAMINA_${U}_API`];
  if (process.env[`DREAMINA_${U}_TOKEN`]) env.token = process.env[`DREAMINA_${U}_TOKEN`];
  if (process.env[`DREAMINA_${U}_APPID`]) env.appId = process.env[`DREAMINA_${U}_APPID`]; // MoreLogin 等需 appId/secret 签名
  if (process.env[`DREAMINA_${U}_SECRET`]) env.secret = process.env[`DREAMINA_${U}_SECRET`];
