...
// Dreamina 默认 adapter（已迁至 0.0.4/playwright/stages/S1-entry/entry-adapter.js）。
// 注意：默认仅作 fallback——register/job-runner 一律通过 options.adapter 注入，
// 故此处用「惰性 require」避免在模块加载期硬耦合 site-specific 文件。
let _dreaminaAdapterCache = null;
function loadDreaminaEntryAdapter() {
  if (!_dreaminaAdapterCache) {
    _dreaminaAdapterCache = require('../../playwright/stages/S1-entry/adapter');
  }
  return _dreaminaAdapterCache;
}
...
function resolveSiteAdapter(siteProfile = {}, options = {}) {
  ...
  if (siteName === 'dreamina') {
    return loadDreaminaEntryAdapter();
  }
