...
const CONFIG_DOCTOR_CFG = path.join(RUNNER_DIR, '..', 'config', 'config.json');
...
if (!skipPreflight) {
  ...
  // 1c. config-doctor（内联）
  try {
    const { diagnoseConfigFile, printDiagnosticReport } = require('../lib/utils/config-doctor');
...
// 找最新 result 文件
const cliResultsDir = path.join(RUNNER_DIR, '..', 'data', 'results');
