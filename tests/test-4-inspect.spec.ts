...
test('inspect test-4 post-click state', async ({ page }) => {
  ...
  const state = await page.evaluate(() => {
    ...
    const continueNode = Array.from(document.querySelectorAll('body *')).find(node => normalize((node as HTMLElement).innerText || '').includes('Continue with email')) || null;
    ...
    return {
      ...
      checkboxClass: normalize((checkboxLabel as HTMLElement | null)?.className || ''),
      bodyPreview: bodyText.slice(0, 500),
    };
  });

  console.log('TEST4_POST_CLICK_STATE=' + JSON.stringify(state));
});
