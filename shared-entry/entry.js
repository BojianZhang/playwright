// ═══════════════════════════════════════════════════════════════════════
// 框架层（FRAMEWORK LAYER）— S1 entry
//
// 文件定位：shared-entry/entry.js
//
// 边界说明（BOUNDARY）：
// ✅ 负责 —— 入口页健康检查阶段调度器。
...
// 依赖方：Dreamina/0.0.3/Sn-xxx/ 下对应的 adapter（调用 entry-adapter / site-adapter（当前绑定 Dreamina），探测入口页就绪信号并返回 EntryResult）
// ═══════════════════════════════════════════════════════════════════════
'use strict';
...
function pickEntryObserveTrace(...candidates) {
...
function pickEntryObserverVersion(...candidates) {
  for (const candidate of candidates) {
  ...
  if (entryOpenResult && entryOpenResult.ok === false) {
    ...
    return normalizeEntryStageResult({
      ...
      detail: {
        ...
        recoveryTrace: null,
        entryOpenResult,
        entryOpenTrace: entryOpenResult?.detail?.openTrace || null,
        observerVersion: pickEntryObserverVersion(
          entryOpenResult?.detail?.openTrace?.observerVersion,
          entryOpenResult?.detail?.whiteScreenObserveTrace?.observerVersion
        ),
        observeTrace: pickEntryObserveTrace(
          entryOpenResult?.detail?.whiteScreenObserveTrace
        ),
  ...
  if (entryHealthResult && entryHealthResult.ok === false) {
    ...
    return normalizeEntryStageResult({
      ...
      detail: {
        ...
        entryOpenTrace: entryOpenResult?.detail?.openTrace || null,
        observerVersion: pickEntryObserverVersion(
          entryHealthResult?.observerVersion,
          entryHealthResult?.observeTrace?.observerVersion,
          entryOpenResult?.detail?.openTrace?.observerVersion,
          entryOpenResult?.detail?.whiteScreenObserveTrace?.observerVersion
        ),
        observeTrace: pickEntryObserveTrace(
          entryHealthResult?.observeTrace,
          entryOpenResult?.detail?.whiteScreenObserveTrace
        ),
        entryHealthResult,
        entryReadyResult: null,
  ...
  if (entryReadyResult?.ok) {
    ...
    return normalizeEntryStageResult({
      ...
      detail: {
        ...
        entryOpenTrace: entryOpenResult?.detail?.openTrace || null,
        observerVersion: pickEntryObserverVersion(
          entryReadyResult?.detail?.observeTrace?.observerVersion,
          entryReadyResult?.detail?.postClickGateTransition?.observeTrace?.observerVersion,
          entryOpenResult?.detail?.openTrace?.observerVersion,
          entryOpenResult?.detail?.whiteScreenObserveTrace?.observerVersion
        ),
        observeTrace: pickEntryObserveTrace(
          entryReadyResult?.detail?.observeTrace,
          entryReadyResult?.detail?.postClickGateTransition?.observeTrace,
          entryOpenResult?.detail?.whiteScreenObserveTrace
        ),
        entryHealthResult,
        entryReadyResult,
...
module.exports = {
  ...
  pickEntryObserveTrace,
  pickEntryObserverVersion,
