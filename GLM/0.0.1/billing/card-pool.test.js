...
// declined 单次只冷却不禁卡。用 GLM_CARD_POOL_FILE 指向临时卡池,绝不碰生产 data/card-pool.json。
'use strict';
...
process.env.GLM_CARD_POOL_FILE = TMP;
...
// ── 充值容量账本 + 原子预留(并发安全)─────────────────────────────────────
test('cardChargeCapacity:次数优先 / 金额算 / 未跟踪不限', () => {
  assert.strictEqual(cp.cardChargeCapacity({ chargeCap: 3, balance: 100 }, 5), 3, '填了次数优先');
  assert.strictEqual(cp.cardChargeCapacity({ balance: 20 }, 5), 4, '金额 floor(20/5)=4');
