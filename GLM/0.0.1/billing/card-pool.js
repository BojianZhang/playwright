...
// 默认指向 data/card-pool.json;GLM_CARD_POOL_FILE 可覆盖(仅供单测指向临时卡池,不污染生产数据)。默认行为不变。
const POOL_FILE = process.env.GLM_CARD_POOL_FILE || path.join(__dirname, '..', 'data', 'card-pool.json');
...
function reapStaleInflight(maxAgeMs) {
  ...
  return mutex(() => lockedWrite(() => {
    let reaped = 0;
    for (const c of POOL.values()) {
      if ((Number(c.chargeInflight) || 0) <= 0) continue;
      const at = Number(c._inflightAt);
      // ★DURABILITY-5:_inflightAt 缺失/非法/【在未来】(时钟倒退或落盘损坏)→ elapsed 会算成负数,
      //   旧式 (now-at)>=age 永远 false → 该卡 inflight 永不回收 = 容量【永久冻结】。
      //   修:未来/非法时戳一律视为已过期立即回收(只清在飞预留计数,不碰 chargedTotal → 无双扣风险,正是 reap 的设计意图)。
      const elapsed = (!c._inflightAt || !Number.isFinite(at)) ? Infinity : (now - at);
      if (elapsed < 0 || elapsed >= age) {
