...
def mac_activate(title_substr=None):
    """把【窗口标题含 title_substr】的进程提到前台(macOS)。需辅助功能权限。返回 True/False。
       遍历可见进程的窗口,命中标题子串就 set frontmost —— 用于 Fix B 在发 OS 级键鼠前确保浏览器在前台。
       title_substr 省略时取环境变量 GLM_WINDOW_TITLE_SUBSTR,否则默认 'z.ai'(GLM 目标站,非旧 OpenRouter)。"""
    if title_substr is None:
        title_substr = os.environ.get("GLM_WINDOW_TITLE_SUBSTR") or "z.ai"
