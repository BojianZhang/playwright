...
def load_config():
    ...
    return {
        "captcha_key": os.environ.get("GLM_CAPTCHA_KEY") or cap.get("apiKey", ""),
        "captcha_provider": os.environ.get("GLM_CAPTCHA_PROVIDER") or cap.get("provider", "twocaptcha"),
        "captcha_timeout": float(cap.get("solveTimeoutMs", 120000) or 120000) / 1000.0,
        "mail_key": os.environ.get("GLM_FIRSTMAIL_KEY") or mb.get("apiKey", ""),
        "mail_base": os.environ.get("GLM_FIRSTMAIL_BASE") or mb.get("apiBaseUrl", "https://firstmail.ltd"),
        "mail_timeout": mail_timeout,
        # 订阅:套餐价格矩阵 + 默认 plan/cycle(pipeline._price_for / steps_subscribe 读)。
        "subscribe_prices": sub.get("prices", {}) or {},
        "subscribe_default_plan": sub.get("defaultPlan", "pro"),
        "subscribe_default_cycle": sub.get("defaultCycle", "monthly"),
    }
