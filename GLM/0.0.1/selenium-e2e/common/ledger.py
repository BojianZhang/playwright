...
def mark_proxy_used(proxy):
    """标记该代理【已被分配跑过号】—— 供"优先用没用过的 IP"(order_proxies 把没 used 的排最前)。并发安全。"""
    if not proxy:
        return
    key = "%s:%s" % (proxy.get("host"), proxy.get("port"))
    with _PROXY_LOCK, _file_lock(paths.PROXY_STATS_FILE):
        try:
            ...
            stats = {}
        stats.setdefault(key, {})["used"] = True
        _atomic_write_json(paths.PROXY_STATS_FILE, stats)


def order_proxies(proxies, start_idx=0):
    """★按偏好给代理池排序(实现"滑块过不去就标记该 IP、后续优先用没用过的 IP"):
       档0=【从没用过】(fresh,最优) → 档1=用过但没被滑块封 → 档2=【滑块封过】(拖到正确位置仍过不去=该IP疑被行为检测盯上,最差兜底才用)
       → 档4=退役(dead 连击)。同档内按 start_idx 轮转(并发多 worker 不抢同一个 fresh IP)。
       无任何战绩(首次)→ 全部 fresh 同档 → 等价原 start_idx 轮转,行为不变。返回排好序的 proxies 列表(不删任何项,只重排)。"""
    if not proxies:
        return proxies
    try:
        with _PROXY_LOCK, _file_lock(paths.PROXY_STATS_FILE):
        ...
        stats = {}
    burn_after = _envint("SLIDER_BURN_AFTER", 3)   # ★1→3:滑块失败含"页面空白"(并发过载,非IP的锅)→ 单次就封会误杀好IP;容3次再降级
    retire_streak = _envint("PROXY_RETIRE_STREAK", 5)

    def _tier(p):
        s = stats.get("%s:%s" % (p.get("host"), p.get("port")), {})
        if s.get("fail_streak", 0) >= retire_streak:
            return 4                                      # 退役 → 最末
        if s.get("slider-pass", 0) <= 0 and s.get("slider-fail", 0) >= burn_after:
            return 2                                      # 滑块封(无通过记录 + 失败达阈值)
        if not s.get("used"):
            return 0                                      # 从没用过 = 最优
        return 1                                          # 用过、未封
    ...
    for t in (0, 1, 2, 4):
        ...
        if not lst:
            continue
        r = start_idx % len(lst)
        out += lst[r:] + lst[:r]                          # 同档内 start_idx 轮转 → 并发分散
    return out
...
def commit_charge(card_id, amount):
    ...
    ★返回 bool:True=已持久化;False=读卡池/落盘失败(调用方应在结果里标记,供人工核对)。无 card_id 返回 True(未跟踪卡,无需记账)。"""
    if not card_id:
        return True
    with _CARD_LOCK, _file_lock(paths.POOL_FILE):
        try:
...
__all__ = [
    "_PROXY_LOCK", "proxy_ok", "mark_proxy_result", "proxy_retired", "mark_proxy_used", "order_proxies",
