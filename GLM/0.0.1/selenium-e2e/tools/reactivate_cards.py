...
def main():
    # ★M14:整段【读→改→原子写】在跨进程文件锁内完成(reactivate 原是唯一没加锁的池写入者)→ 杜绝覆盖
    #   引擎(common/ledger.py)并发写的 usedCount/declineCount 增量。dry-run 也在锁内读,保证看到一致快照。
    with common._file_lock(common.POOL_FILE):
