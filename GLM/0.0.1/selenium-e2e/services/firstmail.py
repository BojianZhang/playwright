...
def get_latest_message(email, password, api_key, base_url=DEFAULT_BASE, folder="INBOX", timeout=12):
    ...
    return http_post_json(
        base_url.rstrip("/") + "/api/v1/email/messages/latest",   # ★去尾斜杠:firstmail.ltd//api → 404(实测复现)
...
def extract_zai_verify_link(msg):
    """从邮件 JSON 抽 z.ai 注册验证链接 https://chat.z.ai/auth/verify_email?token=...（注册是链接验证，非验证码）。"""
    import json as _json
    text = _json.dumps(msg, ensure_ascii=False)
    # 邮件 HTML 里链接可能带转义反斜杠 / &amp;;先精确匹配 verify_email,兜底匹配 chat.z.ai/auth/verify*。
    m = re.search(r'https:\\?/\\?/chat\.z\.ai\\?/auth\\?/verify_email[^"\\\s)]+', text)
    if not m:
        m = re.search(r'https:\\?/\\?/chat\.z\.ai\\?/auth\\?/verify[^"\\\s)]+', text)
    if not m:
        return None
...
# 兼容别名(万一别处按旧名调用)
extract_clerk_verify_link = extract_zai_verify_link
...
def wait_verify_link(email, password, api_key, base_url=DEFAULT_BASE, attempts=14, interval=3, alt_password="", status=None, since_ts=0, skew_ms=60000):
    ...
    for i in range(attempts):
        ...
        if msg is not None:
            ...
            link = extract_zai_verify_link(msg)
            if link:
                ts = _msg_date_ms(msg)
...
def change_mailbox_password(email, current, new, api_key, base_url=DEFAULT_BASE, timeout=30):
    ...
    url = base_url.rstrip("/") + "/api/v1/email/password/change/"   # ★去尾斜杠防 //api → 404
