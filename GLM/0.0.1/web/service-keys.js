...
function pickCaptcha() {
  ...
  return { provider: c.provider || 'capsolver', apiKey: (c.apiKey && String(c.apiKey).trim()) || process.env.GLM_CAPTCHA_KEY || '', source: 'config' };
}
function pickMailbox() {
  ...
  return { provider: m.provider || 'firstmail', apiKey: (m.apiKey && String(m.apiKey).trim()) || process.env.GLM_FIRSTMAIL_KEY || '', apiBaseUrl: m.apiBaseUrl || 'https://firstmail.ltd', source: 'config' };
}
...
function envPatch() {
  ...
  if (cap.apiKey) { env.GLM_CAPTCHA_KEY = cap.apiKey; env.GLM_CAPTCHA_PROVIDER = cap.provider || ''; }
  if (mb.apiKey) { env.GLM_FIRSTMAIL_KEY = mb.apiKey; if (mb.apiBaseUrl) env.GLM_FIRSTMAIL_BASE = mb.apiBaseUrl; }
  return env;
}
