...
// ★逐节点统计:从每行 node_status 聚合每个节点的 {ok, fail, uncertain, total, okRate, reasons}。
//   按主流程顺序排列(注册→登录→取Key→订阅→充值),直接回答用户"每个节点 N成功/M失败,方便统计"。
const NODE_ORDER = [
  'signup_form', 'register_slider', 'create_account', 'verify_email', 'complete_registration', 'register',
  'signin_form', 'login_slider', 'login_submit', 'session_confirm', 'login',
  'oauth_api_click', 'oauth_login_click', 'oauth_signin_form', 'oauth_login_slider', 'oauth_login_submit', 'oauth_session_confirm', 'oauth_login', 'oauth_zai_login',
  'apikey_login', 'apikey_promo', 'apikey_add_dialog', 'apikey_name', 'apikey_create', 'apikey_key_capture', 'apikey',
  'subscribe', 'charge',
];
const NODE_LABELS = {
  signup_form: '注册·填表', register_slider: '注册·滑块', create_account: '注册·提交', verify_email: '注册·邮箱验证',
  complete_registration: '注册·完成', register: '注册·成', signin_form: '登录·填表', login_slider: '登录·滑块',
  login_submit: '登录·提交', session_confirm: '登录·会话确认', login: '登录·成', apikey_login: '取Key·登录态',
  apikey_promo: '取Key·关弹窗', apikey_add_dialog: '取Key·开对话框', apikey_name: '取Key·填名', apikey_create: '取Key·Create',
  apikey_key_capture: '取Key·抓Key', apikey: '取Key·成', subscribe: '订阅', charge: '充值(真扣)',
  oauth_api_click: 'OAuth·点API', oauth_login_click: 'OAuth·点Login', oauth_signin_form: 'OAuth·登录表单', oauth_login_slider: 'OAuth·滑块',
  oauth_login_submit: 'OAuth·提交', oauth_session_confirm: 'OAuth·会话确认', oauth_login: 'OAuth·登录成', oauth_zai_login: 'OAuth·进z.ai',
};
function _nodeStats(rows) {
  const m = new Map();
  for (const r of rows) {
    const ns = r && r.node_status;
    if (!ns || typeof ns !== 'object') continue;
    for (const [node, v] of Object.entries(ns)) {
      const st = (v && v.status) || 'unknown';
      const e = m.get(node) || { node, ok: 0, fail: 0, uncertain: 0, total: 0, _reasons: new Map(), _durs: [] };
      e.total++;
      if (st === 'ok') e.ok++;
      else if (st === 'uncertain') e.uncertain++;
      else { e.fail++; const rs = (v && v.reason) || st; e._reasons.set(rs, (e._reasons.get(rs) || 0) + 1); }
      const d = Number(v && v.dur); if (Number.isFinite(d) && d >= 0) e._durs.push(d);   // ★每环节耗时
      m.set(node, e);
    }
  }
  const order = (n) => { const i = NODE_ORDER.indexOf(n); return i < 0 ? 999 : i; };
  return [...m.values()].sort((a, b) => order(a.node) - order(b.node) || b.total - a.total).map((e) => ({
    node: e.node, label: NODE_LABELS[e.node] || e.node, ok: e.ok, fail: e.fail, uncertain: e.uncertain, total: e.total,
    okRate: e.total ? Math.round(1000 * e.ok / e.total) / 10 : 0,
    avgDur: e._durs.length ? Math.round(10 * e._durs.reduce((a, b) => a + b, 0) / e._durs.length) / 10 : 0,   // 平均耗时(秒)
    maxDur: e._durs.length ? Math.round(10 * e._durs.reduce((mx, d) => (d > mx ? d : mx), 0)) / 10 : 0,        // 最慢一次(秒)→ 看哪步会卡(reduce 取最大,避免 Math.max(...大数组) 栈溢出)
    reasons: [...e._reasons.entries()].map(([label, value]) => ({ label, value })).sort((a, b) => b.value - a.value).slice(0, 8),
  }));
}
...
function analyze(opts) {
  ...
  return {
    ...
    nodeStats: _nodeStats(rows),                                  // ★逐节点 N成功/M失败/uncertain + 失败原因分布(主流程顺序)
