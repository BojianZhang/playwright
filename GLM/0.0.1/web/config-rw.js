...
function secretsState() {
  const m = readMerged();
  return {
    captchaKeySet: !!(getPath(m, 'captcha.apiKey') || process.env.GLM_CAPTCHA_KEY),
    mailboxKeySet: !!(getPath(m, 'mailbox.apiKey') || process.env.GLM_FIRSTMAIL_KEY),
    tokenSet: !!(getPath(m, 'security.token') || process.env.GLM_AUTH_TOKEN),
