...
function pythonBin() {
  return process.env.GLM_PYTHON || (process.platform === 'win32' ? 'python' : 'python3');
}
...
function isSuccessRow(r) {
  if (!r) return false;
  const s = r.steps || {};
  if (s.auth != null && s.auth !== 'ok') return false;                 // 注册/登录失败
  if (s.apikey != null && s.apikey !== true) return false;             // 创建 API Key 跑了没成 → 失败
  const sub = s.subscribe;                                             // 订阅:成功=success;dryrun(未真扣)视为通过
  if (sub != null && sub !== 'success' && sub !== 'dryrun') return false;
  return r.ok === true || !!r.api_key || sub === 'success';            // 通过所有已运行节点 + 确有产出
}
...
function pyFailReason(r) {
  ...
  if (s.apikey === false) return 'apikey:' + (r.key_reason || 'fail');         // 创建 API Key 失败
  if (s.subscribe && s.subscribe !== 'success' && s.subscribe !== 'dryrun') return 'subscribe:' + s.subscribe;
  if (isNotAllowed(r)) return 'ACCOUNT_NOT_ALLOWED';
  if (r.error) return String(r.error).slice(0, 80);
  // ★兜底:按 steps 最远到达的阶段说一句人话。
  if (s.auth === 'ok') {
    if (s.subscribe === 'success') return '收尾未完成';
    if (s.apikey === true || r.api_key) return '订阅未完成';
    return '已注册·后续阶段未完成';
  }
  if (s.auth) return 'auth:' + s.auth;                                         // auth 非 ok 又非 fail 前缀
...
function handoffTarget(_row, _sourceEngine) {
  // GLM 单引擎(纯 Selenium),无跨引擎衔接 → 恒 null。保留导出以兼容 server/测试调用点。
  return null;
}
...
// z.ai 进度等级:registered < apikey < subscribed(供账号台账单调护栏防降级)。
function bestSubscribeStatus(r) {
  if (!r) return '';
  const s = r.steps || {};
  const subscribed = r.subscribed;
  if (s.subscribe === 'success' || (subscribed && subscribed !== false)) return 'subscribed';
  if (r.api_key || s.apikey === true) return 'apikey';
  if (r.registered || s.auth === 'ok') return 'registered';
  return '';
}
...
// 订阅结果归一化(消除 subscribed=0 歧义):success / failed / dry-run / skipped / not-attempted。
function purchaseOutcome(r) {
  const s = r.steps || {};
  const sub = s.subscribe;
  const amt = r.subscribed != null ? r.subscribed : (r.amount != null ? r.amount : null);
  if (r.skipped_subscribe) return { status: 'skipped', label: '已订阅(续跑跳过)', amount: amt, reason: '' };
  if (sub === 'dryrun' || r.subscribe_dryrun) return { status: 'dry-run', label: '未真扣(dry-run)', amount: null, reason: '' };
  if (sub === 'success' || (r.subscribed && r.subscribed !== false)) return { status: 'success', label: '成功', amount: amt, reason: '' };
  if (sub === 'insufficient-funds') return { status: 'failed', label: '钱不够', amount: null, reason: '钱不够' };
  if (sub === 'invalid-amount') return { status: 'failed', label: '金额非法', amount: null, reason: 'Invalid amount' };
  if (sub === 'charge-test-capped') return { status: 'failed', label: '测试帽', amount: null, reason: '整批最多真扣次数已满' };
  if (sub != null && sub !== 'success') return { status: 'failed', label: '失败', amount: null, reason: String(sub) };
  return { status: 'not-attempted', label: '未订阅', amount: null, reason: '' };
}
...
function mapRow(r, accByEmail) {
  ...
  if (isSuccessRow(r)) {
    const _subscribed = r.subscribed != null ? r.subscribed : 0;
    const _ss = bestSubscribeStatus(r);
    return ['success', { email: r.email, password: r.password || orig, originalPassword: orig, apiKey: r.api_key || '', apiKeyName: r.api_key_name || '', subscribeStatus: _ss, plan: r.plan || '', cycle: r.cycle || '', subscribed: _subscribed, paymentStatus: po.status, paymentReason: po.reason, cardLast4: r.card_last4 || '', exitIp, proxy: r.proxy || '', durationSec, timings: _timings, createdAt: r.at || '',
      // 兼容别名(模板遗留页按 billingStatus/charged/purchaseStatus 渲染):指向订阅数据。
      billingStatus: _ss, charged: _subscribed, purchaseStatus: po.status, purchaseReason: po.reason, passwordChanged: false }];
  }
  // ★失败号也带【现密码=op_pw(统一密码优先)】+【原密码】,否则重跑这些号拿不到能登录的密码。
  const _stage = r.fail_stage || (r.steps && (typeof r.steps.auth === 'string' && r.steps.auth !== 'ok' ? 'register' : (r.steps.apikey === false ? 'apikey' : (r.steps.subscribe && r.steps.subscribe !== 'success' && r.steps.subscribe !== 'dryrun' ? 'subscribe' : '')))) || '';
  return ['failed', { email: r.email, password: r.password || orig, originalPassword: orig, reason: pyFailReason(r), stage: _stage, attempts: r.crash_restarts != null ? r.crash_restarts : (r.reopen_count || 0), paymentStatus: po.status, paymentReason: po.reason, plan: r.plan || '', cycle: r.cycle || '', blacklisted: isNotAllowed(r), blacklistReason: isNotAllowed(r) ? 'ACCOUNT_NOT_ALLOWED' : '', proxy: r.proxy || '', durationSec, timings: _timings, createdAt: r.at || '',
    purchaseStatus: po.status, purchaseReason: po.reason, failClass: '' }];   // 兼容别名
}
...
async function bridgeToAccountStore(rows) {
  ...
  for (const r of byEmail.values()) {
    try {
      ...
      // ★当前登录密码=op_pw(统一密码优先,Python res 已写)落台账,账号页/续跑回显一致。
      if (r.password) patch.loginPassword = r.password;
      // ★订阅状态单调护栏(防二次扣款):只在新等级【≥ 旧等级】时覆盖(registered<apikey<subscribed)。
      //   否则失败行会把已达标的 subscribed 降级 → 续跑误判未订阅 → 重新订阅扣款(真金白银,不可回滚)。
      const sub = bestSubscribeStatus(r);
      if (sub && accountStore.attainedLevel(sub) >= accountStore.attainedLevel(prior.billingStatus)) patch.billingStatus = sub;
      // 订阅金额:只写【正数】真实订阅额(只升不降,防降级误判)。
      const subscribed = (typeof r.subscribed === 'number') ? r.subscribed : 0;
      if (subscribed > 0) patch.charged = subscribed;
      if (success && r.plan) patch.plan = r.plan;
      if (success && r.cycle) patch.cycle = r.cycle;
...
function parseResultLine(line) {
  const m = line.match(/════\s*结果\s+(\S+?)\s+ok=(\w+)\s+steps=/);   // run.py:  ════ 结果 NAME ok=B steps=...
  if (!m) return null;
  const name = m[1];
  const pyOk = /^true$/i.test(m[2]);
  // 行内 steps 是 Python repr(单引号),提取 auth/apikey/subscribe(apikey 是 bool → True/False)。
  const _grab = (k) => { const g = line.match(new RegExp("['\"]" + k + "['\"]\\s*:\\s*['\"]([^'\"]+)['\"]")); return g ? g[1] : null; };
  const _grabBool = (k) => { const g = line.match(new RegExp("['\"]" + k + "['\"]\\s*:\\s*(True|False)")); return g ? g[1] === 'True' : null; };
  const auth = _grab('auth');
  const apikey = _grabBool('apikey');
  const subscribe = _grab('subscribe');
  let ok;
  if (auth && auth !== 'ok') ok = false;                                          // 注册/登录没过
  else if (apikey === false) ok = false;                                          // 创建 API Key 失败
  else if (subscribe && subscribe !== 'success' && subscribe !== 'dryrun') ok = false;  // 订阅没成
  else ok = pyOk;
  return { name, ok };
}
...
function reasonFromLine(line) {
  try {
    let m = line.match(/['"]auth['"]\s*:\s*['"](fail[^'"]*)['"]/i);   // 注册/登录失败:fail:SLIDER_FAIL 等
    if (m) return m[1];
    if (/['"]apikey['"]\s*:\s*(False|false)\b/.test(line)) return 'apikey:false';   // 创建 API Key 失败
    m = line.match(/['"]subscribe['"]\s*:\s*['"]([^'"]+)['"]/i);       // 订阅态(success/dryrun 不进失败分支)
    if (m && m[1] !== 'success' && m[1] !== 'dryrun') return 'subscribe:' + m[1];
...
function seleniumArgs(accFile, pxFile, p, jobId) {
  ...
  // 创建 API Key(默认开;独立于订阅)
  a.push(p.doApiKey === false ? '--no-apikey' : '--do-apikey');
  // 订阅 GLM Coding Plan(选套餐 + 信用卡支付)
  if (p.doSubscribe) {
    a.push('--do-subscribe');
    if (p.plan) a.push('--plan', String(p.plan));     // lite|pro|max
    if (p.cycle) a.push('--cycle', String(p.cycle));  // monthly|quarterly|yearly
    // 真实支付:开 → --real-charge(真点 Confirm)+ --card-charge-gate(卡容量账本原子预留);关=dry-run 不真扣。
    ...
    if (p.limitByCapacity) a.push('--limit-by-capacity');
  }
  // 统一密码=z.ai 账号密码:设了就下发(不设=用邮箱密码)。
  if (p.unifiedPassword) a.push('--unified-pw', p.unifiedPassword);
  if (p.keyName) a.push('--key-name', String(p.keyName));
...
function buildEnv(p) {
  ...
  if (p.captchaApiKey) { env.GLM_CAPTCHA_KEY = String(p.captchaApiKey); if (p.captchaProvider) env.GLM_CAPTCHA_PROVIDER = String(p.captchaProvider); }
  if (p.mailboxApiKey) { env.GLM_FIRSTMAIL_KEY = String(p.mailboxApiKey); if (p.mailboxApiBaseUrl) env.GLM_FIRSTMAIL_BASE = String(p.mailboxApiBaseUrl); }
  ...
  try {
    ...
    if (chosen) {
      env.GLM_ADSPOWER_API = chosen.apiBase;
      if (chosen.apiKey && String(chosen.apiKey).trim()) env.GLM_ADSPOWER_TOKEN = String(chosen.apiKey).trim();
    } else {
      const ap = (configRw.readMerged() || {}).adspower || {};
      if (ap.apiBase && String(ap.apiBase).trim()) env.GLM_ADSPOWER_API = String(ap.apiBase).trim();
      if (ap.apiKey && String(ap.apiKey).trim()) env.GLM_ADSPOWER_TOKEN = String(ap.apiKey).trim();
    }
  } catch (_e) { /* 配置读失败用默认端点 */ }
  ...
  // 滑块校准(z.ai;空=Python 内置默认 → 不注 env、逐字节不变)。真机首跑按日志实测值校准。
  const _sScale = Number(p.sliderScale);
  if (p.sliderScale !== undefined && p.sliderScale !== '' && Number.isFinite(_sScale) && _sScale > 0) env.SLIDER_SCALE = String(_sScale);
  const _sOff = Number(p.sliderOffset);
  if (p.sliderOffset !== undefined && p.sliderOffset !== '' && Number.isFinite(_sOff)) env.SLIDER_OFFSET = String(_sOff);
  const _sAtt = Number(p.sliderAttempts);
  if (p.sliderAttempts !== undefined && p.sliderAttempts !== '' && Number.isFinite(_sAtt) && _sAtt > 0) env.SLIDER_ATTEMPTS = String(Math.floor(_sAtt));
  // 缺口识别策略(空=默认 本地+Cap 交叉、分歧走 Cap;不注 env=逐字节默认行为)。模式互斥,select 保证只设一个。
  //   capsolver-verified=以 CapSolver 值【最终交付】、本地精准检测(sat+ncc)当裁判:一致才提交 Cap 值,否则刷新重取求一致。
  if (p.sliderGapMode === 'local-first') env.SLIDER_LOCAL_FIRST = '1';
  else if (p.sliderGapMode === 'pure-capsolver') env.SLIDER_PURE_CAPSOLVER = '1';
  else if (p.sliderGapMode === 'capsolver-verified') env.SLIDER_GAP_MODE = 'capsolver-verified';
  const _sConf = Number(p.sliderLocalMinConf);
  if (p.sliderLocalMinConf !== undefined && p.sliderLocalMinConf !== '' && Number.isFinite(_sConf) && _sConf > 0) env.SLIDER_LOCAL_MIN_CONF = String(_sConf);
  // 验证模式参数:cap↔本地一致容差(原生px,空=6)+ 不一致最多刷新几次求一致(空=3)。仅 capsolver-verified 用,不注=Python 默认。
  const _vTol = Number(p.sliderVerifyTol);
  ...
  //   合成单个 GLM_RECOVERY_JSON,run.py AUTO_RETRY 按 fail_stage 决定各失败类型是否重试。
  ...
  try {
    ...
    if (_recJson && _recJson !== '{"retry":{}}') env.GLM_RECOVERY_JSON = _recJson;
...
function runSpec(jobId, spec, publish, shared) {
  ...
  return new Promise((resolve) => {
    ...
    if (!child.pid) { publish(jobId, 'log', `${spec.label}未拿到 pid(python 未安装?设 GLM_PYTHON)`); shared.exits.push({ started: false }); return resolve(); }
...
async function spawnEngine(jobId, engine, payload, publish) {
  ...
  engine = 'selenium';   // ★GLM 单引擎(纯 Selenium / run.py);下方 split/hybrid 分支为继承自模板的死代码,永不触达。
  ...
  try {
    if (engine === 'split') {
      ...
      const envA = { ...env, GRID_TOTAL: String(_c * 2), GRID_SLOT_OFFSET: '0', ...(_ho ? { GLM_FAST_HANDOFF_KEY: '1' } : {}) };
      const envB = { ...env, GRID_TOTAL: String(_c * 2), GRID_SLOT_OFFSET: String(_c), ...(_ho ? { GLM_FAST_HANDOFF_CARD: '1' } : {}) };
...
function adspowerSelftest({ timeoutMs = 90000 } = {}) {
  return new Promise((resolve) => {
    ...
    if (!child.pid) return finish({ ok: false, detail: '未拿到 pid(python 未安装?设环境变量 GLM_PYTHON)' });
