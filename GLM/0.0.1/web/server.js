...
// GLM 单引擎(纯 Selenium,走 /api/run → engine-runner.spawnEngine)。无 Playwright 引擎 →
// jobRunner 仅为兼容模板里 /jobs 路由的存根;真被调用即明确报错(前端不会走 /jobs)。
const jobRunner = { runJob: async () => { throw new Error('GLM 为纯 Selenium 单引擎,无 Playwright /jobs;请用 /api/run(engine=selenium)'); } };
...
const PORT = Number(process.env.GLM_WEB_PORT) || 4317;
...
const NODE_ID = (process.env.GLM_NODE_ID || os.hostname() || 'node').replace(/[^\w-]/g, '-').slice(0, 40);
...
function loadClusterHosts() {
  ...
  if (process.env.GLM_CLUSTER_HOSTS) hosts = process.env.GLM_CLUSTER_HOSTS.split(',').map((s) => s.trim()).filter(Boolean);
...
function getCentralUrl() { return process.env.GLM_CENTRAL_URL || readCluster('centralUrl') || ''; }
...
// 令牌来源:环境变量 GLM_AUTH_TOKEN > config.local.json/config.json 的 security.token。
function loadAuthToken() {
  if (process.env.GLM_AUTH_TOKEN) return process.env.GLM_AUTH_TOKEN;
  for (const f of ['config.local.json', 'config.json']) {
...
const AUTH_USER = process.env.GLM_WEB_USER || '';
const AUTH_PASS = process.env.GLM_WEB_PASS || '';
...
function pingAdspower(apiBase, apiKey) {
  return new Promise((resolve) => {
    ...
    if (tok) {
      const hn = process.env.GLM_ADSPOWER_AUTH_HEADER || 'Authorization';
      const pfx = process.env.GLM_ADSPOWER_AUTH_PREFIX != null ? process.env.GLM_ADSPOWER_AUTH_PREFIX : 'Bearer ';
      headers[hn] = pfx + tok;
    }
    const t0 = Date.now(); let done = false;
...
function handleApiHealth(res) {
  ...
  const warnings = [];
  if (!cfg.captchaKeySet) warnings.push('未配置验证码(captcha)key —— Turnstile/hCaptcha 无法自动求解(设置中心填,或环境变量 GLM_CAPTCHA_KEY)');
...
// ── 服务器 ──────────────────────────────────────────────────────────────
const HOST = process.env.GLM_WEB_HOST || '0.0.0.0';

// 仅"数据/动作"接口需要鉴权;静态页面(无敏感数据,只是 UI 外壳)放行,
// 这样开了 token 也能先打开页面、再由前端弹窗输入 token。
// 全锁定模式:开启后连静态页面也要 token(陌生人有域名也打不开)。
// 用 ?token= 访问一次即可(auth.js 会带入并记住)。开关:security.gateStatic / GLM_GATE_STATIC=1
function loadGateStatic() {
  if (process.env.GLM_GATE_STATIC) return process.env.GLM_GATE_STATIC === '1' || process.env.GLM_GATE_STATIC === 'true';
  for (const f of ['config.local.json', 'config.json']) {
...
const ALLOW_IPS = process.env.GLM_ALLOW_IPS ? toList(process.env.GLM_ALLOW_IPS) : toList(readSec('allowIps'));
const ALLOW_HOSTS = (process.env.GLM_ALLOW_HOSTS ? toList(process.env.GLM_ALLOW_HOSTS) : toList(readSec('allowHosts'))).map((h) => h.toLowerCase());
const TRUST_PROXY = process.env.GLM_TRUST_PROXY === '1' || readSec('trustForwardedFor') === true;
...
function gracefulShutdown(signal) {
  ...
  try { console.log(`[GLM Web] 收到 ${signal} → 刷盘 + 清子进程后退出`); } catch (_e) { /* ignore */ }
  flushAllStores();
...
function doRestart() {
  ...
  try { console.log('[GLM Web] 一键重启 → 刷盘 → 拉起新实例 → 退出旧实例'); } catch (_e) { /* */ }
...
const onListening = () => {
  ...
  console.log(`[GLM Web] 控制台已启动: http://${HOST}:${PORT}  (本机: http://localhost:${PORT})`);
  ...
  //   而 X-Forwarded-For 客户端可伪造 → 任意人伪造白名单 IP 即可绕过。务必同时配 GLM_AUTH_TOKEN(token 才是主关卡)。
  if (TRUST_PROXY && ALLOW_IPS.length && !AUTH_TOKEN && !AUTH_USER) {
    console.warn('⚠⚠ [安全] 已开 TRUST_PROXY + IP白名单 但【未配 token/密码】→ X-Forwarded-For 可被伪造绕过 IP 白名单!请配 GLM_AUTH_TOKEN,或仅在【会清洗 XFF 的可信反代】后开 TRUST_PROXY。');
  }

  // 子机自动注册:配了中心机地址就启动时 + 每 30s 心跳上报,中心机自动聚合本机。
  const CENTRAL_URL = getCentralUrl();
  const SELF_URL = process.env.GLM_SELF_URL || readCluster('selfUrl') || '';
  if (CENTRAL_URL) {
  ...
  if (AUTH_TOKEN) {
    ...
    console.log('   请任选其一加固: 设 GLM_AUTH_TOKEN=一个随机串(推荐) / 绑定内网+防火墙 / GLM_WEB_HOST=127.0.0.1 + SSH 隧道。');
  }
};
...
server.on('error', (e) => {
  if (e && e.code === 'EADDRINUSE' && _bindTries < 40) {   // 最多 ~20s 重试,覆盖旧实例 SSE 长连释放端口的窗口
    _bindTries += 1;
    if (_bindTries === 1) console.log(`[GLM Web] 端口 ${PORT} 被占用(旧实例退出中?)→ 重试绑定…`);
    if (_bindTries === 40) console.error(`[GLM Web] ⚠ 端口 ${PORT} 重试 20s 仍被占用,最后一次尝试…`);
    setTimeout(() => server.listen(PORT, HOST, onListening), 500);
  } else {
    console.error('[GLM Web] 监听失败:', (e && e.message) || e);
    process.exit(1);
  }
});
