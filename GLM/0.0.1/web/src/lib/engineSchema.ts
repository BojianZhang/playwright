...
// 验证码【加载】超时(治"页面元素经常加载不全、Click to start verification 出不来")——刷页是加载的敌人,这几个控制等多久才放弃刷页
const Z_SLIDER_OPENWAIT: EngineField = { key: 'sliderOpenWait', label: '验证码加载总等待(s)', hint: '打开验证码浮层的总超时上限;空=默认30。代理慢/「页面元素经常加载不全」时调大', type: 'number', default: '', min: 5, step: 1 };
const Z_SLIDER_NOCTRL_GRACE: EngineField = { key: 'sliderNoControlGrace', label: '无验证码就刷页等待(s)', hint: '页面【完全没有】验证码SDK/脚本时,等几秒判定"真没加载"就刷当前页重载;空=默认12。★就是你说的"8秒刷太快"那个值,调大=更耐心少刷', type: 'number', default: '', min: 3, step: 1 };
const Z_SLIDER_NOCTRL_HARDCAP: EngineField = { key: 'sliderNoControlHardcap', label: '验证码加载硬上限(s)', hint: '验证码SDK【在、但触发器迟迟没渲染】时,最多耐心等几秒(期间强制催渲染、绝不刷页)再放弃;空=默认25。SDK在就别刷——刷会把没加载完的冲掉重来', type: 'number', default: '', min: 5, step: 1 };
// 优化开关(数据驱动,带逃生开关——稳定后再决定去留)
const Z_APIKEY_WAITCONTENT: EngineField = { key: 'apikeyWaitContent', label: '取key页等内容渲染', hint: '取key页常黑屏(URL对但只剩logo+汉堡,内容没渲染)→ 默认先等真内容渲染、好了就往下(更快);黑屏超时快速失败(不再干等22s找不存在的Add按钮)。关掉=老逻辑', type: 'bool', default: true };
const Z_APIKEY_CONTENTWAIT: EngineField = { key: 'apikeyContentWait', label: '取key内容等待(s)', hint: '上面开启时,等取key页内容渲染的超时;空=默认12', type: 'number', default: '', min: 3, step: 1 };
const Z_APIKEY_FORCERELOAD: EngineField = { key: 'apikeyForceReload', label: '取key黑屏硬刷', hint: '取key页黑屏催渲染时额外 location.reload(true) 硬刷一次(更激进,实验);默认关', type: 'bool', default: false };
const Z_ACCT_DEADLINE: EngineField = { key: 'accountDeadline', label: '单号超时放弃(s)', hint: '单个号总耗时超此秒数就放弃、换下一个(防个别号卡死无限重试占名额、拖垮整批吞吐);空/0=不限。建议 240~300', type: 'number', default: '', min: 0, step: 10 };
const Z_SLIDER_STRICT: EngineField = { key: 'sliderStrictConsensus', label: '滑块严格共识', hint: '只在 sat 与 ncc 两个本地法【互相一致】时才拖滑块,少拖那些"拖准了缺口却判偏"的低共识值(占FAIL约69%);默认关=当前行为', type: 'bool', default: false };
...
export const ENGINE_FIELDS: Record<EngineKey, EngineField[]> = {
  ...
  selenium: [Z_DOAPIKEY, Z_DOSUB, Z_PLAN, Z_CYCLE, Z_REALCHARGE, Z_CHARGECNT, Z_LIMITCAP, F_AUTORETRY, F_AUTORETRY_N, F_NODEL, Z_SLIDER_GAPMODE, Z_SLIDER_MINCONF, Z_SLIDER_VERIFY_TOL, Z_SLIDER_VERIFY_REFRESH, Z_SLIDER_SCALE, Z_SLIDER_OFFSET, Z_SLIDER_ATT, Z_SLIDER_OPENWAIT, Z_SLIDER_NOCTRL_GRACE, Z_SLIDER_NOCTRL_HARDCAP, Z_APIKEY_WAITCONTENT, Z_APIKEY_CONTENTWAIT, Z_APIKEY_FORCERELOAD, Z_ACCT_DEADLINE, Z_SLIDER_STRICT],
