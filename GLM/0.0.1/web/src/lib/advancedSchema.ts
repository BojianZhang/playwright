...
export const ADV_FIELDS: AdvField[] = [
  // ── 提速总开关(默认关=与现状逐字节一致)──
  { key: 'fastMode', env: 'GLM_FAST', label: '提速模式 ★', hint: '空=关(默认,与现状完全一致);on=开:注册/登录省成功路径截图 + 把固定等待改成轮询提前退出(更快,不改成功判定/反检测)', type: 'select', scope: 'both', group: '提速', def: '', options: ['', 'on'] },
  ...
  // ── 驱动 / 环境(两套共用)──
  { key: 'maxConcurrency', env: 'GLM_MAX_CONCURRENCY', label: '并发硬上限 ★', hint: '防 AdsPower 批量掉线:任何 job 的每进程并发超过它都自动钳到它(无论控制台填多少),设一次以后都生效。留空=不限。建议先 4~6,稳了(无 session-deleted)再逐步往上加', type: 'number', scope: 'both', group: '驱动环境', def: '' },
  ...
  { key: 'cardBinDailyCap', env: 'CARD_BIN_DAILY_CAP', label: 'BIN 当日上限', hint: '每个 BIN 一天最多绑几个号(降 Stripe Radar velocity);留空=默认 20。超额后自动轮换到当日发号最少的 BIN,绝不饿卡', type: 'number', scope: 'both', group: '卡池', def: '' },
  // ── 充值 / 改密(计费收尾)──
