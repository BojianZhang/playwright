...
// 逐节点状态统计(主流程每个节点 N成功/M失败/uncertain + 失败原因分布)。
export interface NodeStat { node: string; label: string; ok: number; fail: number; uncertain: number; total: number; okRate: number; avgDur: number; maxDur: number; reasons: AnalyticsBucket[]; }
