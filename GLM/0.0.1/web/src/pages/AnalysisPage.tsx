    ...
    <main className="page">
      ...
      {data && (
        <>
          ...
          {/* ★逐节点状态:主流程每个节点 N成功/M失败/uncertain + 失败原因 */}
          {!!(data.nodeStats && data.nodeStats.length) && (<>
            <div className="section-gap" />
            <section className="card">
              <div className="eb-top"><span className="idx c-blue">⊞</span><h3>逐节点状态(成功/失败统计)</h3>
                <span className="head-hint">主流程每个节点 · 条=成功率 · ✓成功 ✗失败 ?待核</span>
              </div>
              <div style={{ padding: '10px 18px', display: 'flex', flexDirection: 'column', gap: 3 }}>
                {data.nodeStats!.map((n) => {
                  const pct = n.okRate;
                  const col = pct >= 80 ? 'var(--success)' : pct >= 40 ? 'var(--warn)' : 'var(--danger)';
                  return (
                    <div key={n.node} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '4px 0', borderBottom: '1px solid var(--border)' }}>
                      <div style={{ width: 120, fontSize: 12.5, color: 'var(--text)' }}>{n.label}</div>
                      <div style={{ flex: 1, height: 13, background: 'var(--bg-2, #eee)', borderRadius: 7, overflow: 'hidden' }}>
                        <div style={{ width: `${Math.max(2, pct)}%`, height: '100%', background: col }} />
                      </div>
                      <div style={{ width: 46, textAlign: 'right', fontFamily: 'var(--mono)', fontWeight: 700, color: col }}>{pct}%</div>
                      <div style={{ width: 132, fontSize: 11.5, color: 'var(--text-3)', fontFamily: 'var(--mono)' }}>
                        <span style={{ color: 'var(--success)' }}>{n.ok}✓</span> <span style={{ color: 'var(--danger)' }}>{n.fail}✗</span>{n.uncertain ? <span style={{ color: 'var(--warn)' }}> {n.uncertain}?</span> : null} · {n.total}
                      </div>
                      <div style={{ width: 96, fontSize: 11, fontFamily: 'var(--mono)', color: (n.maxDur >= 30 ? 'var(--warn)' : 'var(--text-3)') }} title={`平均 ${n.avgDur}s · 最慢 ${n.maxDur}s(看哪步慢/卡)`}>
                        ~{n.avgDur}s<span style={{ color: 'var(--text-4, var(--text-3))' }}> / {n.maxDur}s</span>
                      </div>
                      <div style={{ width: 200, fontSize: 11, color: 'var(--text-3)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }} title={n.reasons.map((r) => `${r.label}:${r.value}`).join(', ')}>
                        {n.reasons.slice(0, 3).map((r) => `${r.label}×${r.value}`).join(' ') || '—'}
                      </div>
                    </div>
                  );
                })}
