...
export default function ConsolePage() {
  ...
  // —— 引擎选择 + Python 引擎高级参数 ——
  const [engine, setEngine] = useState<Engine>('selenium');   // GLM 单引擎(纯 Selenium)
  ...
  function buildRunPayload() {
    ...
    return {
      ...
      unifiedPassword: unifiedPwd.trim(),
      doChangePw: stages.pwd && pwdGateOk,
      ...
      useProxyPool, useAddressPool, useAdspowerPool,
      successTemplate: tplOk, failureTemplate: tplFail,
      // ── z.ai 引擎选项(引擎配置·覆盖上面的 OpenRouter 遗留键)──
      doApiKey: eng.doApiKey !== false,
      doSubscribe: !!eng.doSubscribe,
      plan: String(eng.plan || 'pro'),
      cycle: String(eng.cycle || 'monthly'),
      realCharge: eng.realCharge === true || String(eng.realCharge) === 'true',
      chargeCount: Math.max(0, num(eng.chargeCount, 0)),
      limitByCapacity: !!eng.limitByCapacity,
      sliderScale: eng.sliderScale ?? '', sliderOffset: eng.sliderOffset ?? '', sliderAttempts: eng.sliderAttempts ?? '',
    };
  }
