    ...
    <>
      ...
      <section className="card">
        ...
        <div className={'set-hero' + (c.pwdInvalid && !c.unifiedPwd.trim() ? ' invalid' : '')}>
          ...
          {c.engine === 'playwright' ? (
            <p className="set-hero-help"><b>z.ai 注册就用它</b>,全流程跑完后再把邮箱密码从原密码改成它。<b className="warn">不能留空</b> —— 否则只能按「原邮箱:密码」保存,后续改密会很费劲。</p>
          ) : (
            <p className="set-hero-help">作 <b>z.ai 登录密码</b>。Selenium 仅在勾「改密」时用作目标新密码;混合引擎用作登录密码;<b>留空 = 用每个账号自己的邮箱密码</b>(可不填)。</p>
          )}
        </div>
