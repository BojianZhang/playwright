...
## ✅ 负责
- **卡池单一来源**(`card-pool.js`):卡片行解析、并发安全取卡(acquire)/回报(report)、使用次数/状态/冷却跨任务累计、脱敏快照。落盘 `../account-state/card-pool.json`。
...
- **数据文件位置** → 落在 `../account-state/`,本库只读写不定义目录。

## 安全
`../account-state/card-pool.json` 及备份含**完整卡号/CVC** → 已 gitignore,**绝不入库**;对外 `snapshot()` 一律脱敏(→ `••last4`,无 CVC)。

## 依赖
→ `../account-state/`、仓库根 `../../../shared-batch-orchestration/mutex`。被 `../playwright/stages.js` 与 `../web/server.js` 调用。
