...
test('purchaseOutcome: 充值结果明确归一(成功/失败/已充跳过/未充值),消除 charged=0 歧义', () => {
  ...
  // ★充值容量闸新态:dry-run(未真扣)/ 钱不够 / 测试帽
  assert.strictEqual(er.purchaseOutcome({ charge_dryrun: true, steps: { card: 'card-bound' } }).status, 'dry-run', '真实充值关→dry-run');
  assert.strictEqual(er.purchaseOutcome({ steps: { card: 'card-bound', purchase: 'insufficient-funds' } }).status, 'failed');
  assert.strictEqual(er.purchaseOutcome({ steps: { purchase: 'insufficient-funds' } }).label, '钱不够');
  assert.strictEqual(er.purchaseOutcome({ steps: { purchase: 'charge-test-capped' } }).label, '测试帽');
  // ★isSuccessRow:dry-run(card-bound,无 purchase)=成功;钱不够/测试帽(card-bound 但 purchase 非 success)=失败
  assert.strictEqual(er.isSuccessRow({ ok: true, charge_dryrun: true, steps: { auth: 'ok', card: 'card-bound' } }), true, 'dry-run=card-bound成功');
  assert.strictEqual(er.isSuccessRow({ steps: { card: 'card-bound', purchase: 'insufficient-funds' } }), false, '钱不够=失败');
  assert.strictEqual(er.isSuccessRow({ steps: { card: 'card-bound', purchase: 'charge-test-capped' } }), false, '测试帽=失败');
  // ★parseResultLine 实时同口径:钱不够/测试帽=失败;dry-run(无 purchase,python ok=true)=成功
  assert.strictEqual(er.parseResultLine("════ 结果 a ok=false steps={'auth': 'ok', 'card': 'card-bound', 'purchase': 'insufficient-funds'}").ok, false);
  assert.strictEqual(er.parseResultLine("════ 结果 b ok=true steps={'auth': 'ok', 'card': 'card-bound'}").ok, true, 'dry-run card-bound→成功');
});
