...
const F_RESULT_WAIT: EngineField = { key: 'cardResultWait', label: '绑卡结果等待上限(秒)', hint: '点 Save 后轮询「绑成/被拒」结果的上限;遇 Stripe「Save card?」存卡弹窗会拖到此上限才转去刷新核验。调小=更快收尾(默认24)', type: 'number', default: '24', min: 1, step: 1 };
...
export const ENGINE_FIELDS: Record<EngineKey, EngineField[]> = {
  ...
  // 纯 selenium(run.py→pipeline.py)读 FIXC_SOLVE_HCAPTCHA + FIXC_HC_RECHECK_WAIT + FIXC_RESULT_WAIT(都走 fixc_core);
  // cardDeadline/solveFutileCap/maxHcaptchaCardSwaps 仅 hybrid_run.py 读 → 对 selenium 是死字段,不展示。
  selenium: [F_SOLVE, F_HC_RECHECK, F_RESULT_WAIT, F_STALL, F_NODEL],
  hybrid: [F_SOLVE, F_DEADLINE, F_FUTILE, F_SWAPS, F_HC_RECHECK, F_RESULT_WAIT, ...HS_ROTATE, ...HS_ENVLIFE, F_NODEL],
  split: [F_SOLVE, F_DEADLINE, F_FUTILE, F_SWAPS, F_HC_RECHECK, F_RESULT_WAIT, F_STALL, ...HS_ROTATE, ...HS_ENVLIFE, F_NODEL,
