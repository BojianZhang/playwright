...
function TopBar() {
  ...
  // 一键重启:POST /api/restart → 服务端自重启(拉新实例+退旧)→ 这里轮询直到服务恢复后自动刷新页面。
  async function restartServer() {
    if (restarting) return;
    if (!window.confirm('重启控制台服务?\n\n会断开本页约 2-3 秒后自动重连(用于让后端/产物更新生效)。\n不会中断正在跑的任务。')) return;
    setRestarting(true);
    toast.push('正在重启服务…恢复后自动刷新', 'info');
    try { await apiPost('/api/restart', {}); } catch { /* 旧进程退出导致连接断开属正常,继续等恢复 */ }
    // 轮询 /api/node 直到新实例起来(端口交接 + 启动需要一两秒),回来即刷新
    for (let i = 0; i < 40; i++) {
      await new Promise((r) => setTimeout(r, 700));
      try { await apiGet('/api/node', true); window.location.reload(); return; } catch { /* 仍在重启,继续等 */ }
    }
    setRestarting(false);
    toast.push('等待服务恢复超时,请手动刷新页面', 'err');
  }
  ...
  return (
    <header className="topbar">
      ...
      <button className="btn btn-ghost btn-sm" onClick={restartServer} disabled={restarting} title="重启控制台服务(让后端 / 前端产物更新生效;不中断正在跑的任务)">
        <Icon name="refresh" size={13} />{restarting ? '重启中…' : '重启服务'}
      </button>
