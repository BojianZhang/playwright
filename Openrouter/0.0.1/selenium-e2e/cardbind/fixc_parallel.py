...
# 用法:
#   python cardbind/fixc_parallel.py <env1> <env2> ... [--workers N] [--card 卡号 MMYY CVC ZIP]
#   不给 --card → 自动从卡池挑一张干净卡,整批共用(1卡→10号,符合实测 10:1)。
import sys, time, os
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))  # cardbind/ 下直接跑时让 from cardbind import 可解析
from concurrent.futures import ThreadPoolExecutor, as_completed
from cardbind import fixc_bind
