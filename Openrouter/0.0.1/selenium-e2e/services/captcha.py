...
function w(){try{if(!window.hcaptcha)return;if(window.hcaptcha.render&&!window.hcaptcha.__wr){var o=window.hcaptcha.render.bind(window.hcaptcha);window.hcaptcha.render=function(c,p){g(p);return o(c,p);};window.hcaptcha.__wr=true;}if(window.hcaptcha.execute&&!window.hcaptcha.__we){var oe=window.hcaptcha.execute.bind(window.hcaptcha);window.hcaptcha.execute=function(a,b){try{var opt=(a&&typeof a==='object')?a:b;if(opt&&opt.rqdata){window.__hcParams=window.__hcParams||{};window.__hcParams.rqdata=opt.rqdata;}}catch(e){}var nat=oe(a,b);try{var o2=(a&&typeof a==='object')?a:b;if(o2&&o2.async){return Promise.race([nat,new Promise(function(res){var iv=setInterval(function(){if(window.__hcInjectedToken){clearInterval(iv);res({response:window.__hcInjectedToken,key:window.__hcInjectedToken});}},50);setTimeout(function(){clearInterval(iv);},150000);})]);}}catch(e){}return nat;};window.hcaptcha.__we=true;}}catch(e){}}
...
def solve_hcaptcha(driver, page_url, cfg, timeout=120, patcher=None, proxy=None):
    ...
    # ② 注入 token:关键是【先置 window.__hcInjectedToken】——WRAPPER_HCAPTCHA hook 把 hcaptcha.execute(async)
    #    包成了 Promise.race,会轮询这个值并 resolve 给 Stripe 正在 await 的那个 execute promise(隐形企业版唯一接得回的路);
    #    再调 hcCallback/setResponse/填 textarea 兜住 render-callback 式集成。用 eval_collect 收每个 OOPIF 的命中情况,
    #    让 hadCallback 真实反映"回调是否在某个跨域帧里命中"(eval_all 是 fire-and-forget、永远报不出来)。
    had_patcher = False
    if patcher:
        inject_expr = (
            "(function(){var tok=%s;var cb=false,called=false,ta=0;"
            "try{window.__hcInjectedToken=tok;}catch(e){}"
            "try{cb=(typeof window.hcCallback==='function');if(cb){window.hcCallback(tok);called=true;}}catch(e){}"
            "try{if(window.hcaptcha&&typeof window.hcaptcha.setResponse==='function'){window.hcaptcha.setResponse(tok);}}catch(e){}"
            "try{document.querySelectorAll('textarea[name=\"h-captcha-response\"],textarea[name=\"g-recaptcha-response\"],textarea[id^=\"h-captcha-response\"],textarea[id^=\"g-recaptcha-response\"]').forEach(function(el){el.value=tok;el.dispatchEvent(new Event('input',{bubbles:true}));el.dispatchEvent(new Event('change',{bubbles:true}));ta++;});}catch(e){}"
            "return JSON.stringify({cb:cb,called:called,ta:ta});})()" % json.dumps(token))
        try:
            results = patcher.eval_collect(inject_expr)
            hit_cb = 0
            hit_ta = 0
            for r in results:
                try:
                    o = json.loads(r) if isinstance(r, str) else r
                except Exception:
                    o = None
                if o:
                    if o.get("called"):
                        hit_cb += 1
                    hit_ta += int(o.get("ta") or 0)
            had_patcher = hit_cb > 0
            log("hcaptcha token 已 CDP 注入 %d 个 target(真回调命中 %d,填 textarea %d 处,含 Stripe OOPIF)"
                % (len(results), hit_cb, hit_ta))
        except Exception as e:
            log("hcaptcha CDP 注入异常: %s" % str(e)[:60])
    from selenium.webdriver.common.by import By
    INJECT = ("var tok=arguments[0],called=false;"
              "try{window.__hcInjectedToken=tok;}catch(e){}"   # 同样置位:让本帧 wrapper 的 execute race resolve
    ...
    _inject(0)
    had = had[0] or had_patcher              # had 反映真注入路径(patcher OOPIF 命中也算),不再只看死壳下注定失败的 Selenium 兜底
    try:
        driver.switch_to.default_content()
    except Exception:
        pass
    # token 已注入。后端校验 token + 关模态框要几秒——之前只等 2.5s 就判失败 → 上层换卡 → page.goto 重载把
    # in-flight token 冲掉。这里【绝不判失败触发换卡】(避免重载冲掉 token),即便框暂未消失也返回 True,交调用方
    # (fixc_core 的活 CDP / add_card 的 Save 循环)判 card-bound。
    # ★Fix C 死壳(chromedriver 已被 kill):has_hcaptcha(driver) 恒 False、纯属失明 → 绝不据此假报"挑战消失";
    #   给 wrapper race resolve + Stripe 处理一点时间就交回活 CDP 复检(权威判定在 fixc_core.cdp.evaluate(CAPTCHA_JS))。
    if getattr(driver, "is_dead_shell", False):
        time.sleep(3)
        log("hcaptcha token 已注入(hadCallback=%s);框是否消失交活 CDP 复检(死壳不在此判,避免误报通过)" % had)
        return True
    for _ in range(6):                      # 活 driver(纯 Selenium 路):最多再等 ~15s 看框是否真消失
        time.sleep(2.5)
        try:
