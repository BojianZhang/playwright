...
def cdp_fill_and_save(driver, num, exp, cvc, zipc, log=print, alt_zips=None):
    ...
    if cap:
        ...
        if os.environ.get("FIXC_MANUAL_CAPTCHA") == "1":
            # 手动解框模式(验证用):停下等人手动点 I am human / 解图片,看干净会话解开后绑不绑成
            secs = int(os.environ.get("FIXC_MANUAL_WAIT", "90"))
            log("👉👉 弹验证框!请【手动点 I am human / 解图片】,我等 %d 秒..." % secs)
            time.sleep(secs)
        else:
            # 自动:原生CDP点 I am human(干净会话很可能被动直接过)
            hit = click_i_am_human(cdp, log)
            log("⚠ 弹验证框 → 点 I am human: %s" % (hit or "没找到 checkbox iframe"))
            if hit == "image-challenge":
                # 图片九宫格:干净会话被动点不掉。Fix C 的核心是【绝不开任何 domain】——要 2captcha 抓真回调
                # 必须 addScriptToEvaluateOnNewDocument 装 hook(破坏免检会话,正是 Stripe Radar 502 的根因)。
                # 所以不在这里硬解,直接当 hcaptcha 交上层【换卡/换IP/弃号】(实测有效),不空转、不污染会话。
                image_challenge = True
                cap = True
                log("   → 图片挑战:交上层换卡/换IP(Fix C 不破坏免检会话去硬解);跳过无用的重点Save")
        if not image_challenge:                      # 图片挑战已确定过不去 → 跳过无用的重点Save(省~5s)
            try:
                sv2 = cdp.evaluate(SAVE_JS)               # 解后再点一次 Save 提交
                if sv2:
                    s2 = json.loads(sv2); cdp.mouse_click(s2[0], s2[1]); time.sleep(2.5)
            except Exception:
                pass
            try:
                cap = bool(cdp.evaluate(CAPTCHA_JS))      # 还有框=没解掉
            except Exception:
                pass
        log("   解框后仍有验证框=%s%s" % (cap, "(图片挑战,换卡)" if image_challenge else ""))
    if image_challenge:                              # 图片挑战必过不去 → 立即返回交上层换卡/换IP,别再轮询24s干等
        cdp.close()
