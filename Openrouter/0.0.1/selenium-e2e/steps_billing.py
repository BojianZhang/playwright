...
def _fill_save_cdp(page, card, address, save_timeout=60):
...
def add_card(page, card, address, cfg, manual_hcaptcha=True, save_timeout=60, patcher=None, proxy=None, manual_card=False, card_ref=None, fill_mode="selenium"):
    ...
    has_addpm = page.js(
        "return !!Array.from(document.querySelectorAll('button,[role=button],a')).find(function(b){return /Add a Payment Method|Add Payment Method/i.test(b.innerText||'');})")
    if (not has_addpm) and re.search(r"Auto\s*Top-?Up", t0, re.I):
        log("[加卡] 页面显示 Auto Top-Up 且无 Add a Payment Method → 该号已有支付方式,视为已绑卡")
        return {"result": "card-bound", "detail": "已有支付方式(Auto Top-Up)", "hcaptcha": False}
    ...
    if page.field_present(ADDR_NAME):
        ...
        page.wait_and_fill(['input[name="locality"]', 'input[name="city"]', 'input[autocomplete="address-level2"]'], address["city"], 12, "城市")
        page.wait_and_select(['select[name="administrativeArea"]', 'select[autocomplete="address-level1"]'], address["state"], 10, "州")
        page.wait_and_fill(['input[name="postalCode"]', 'input[autocomplete="postal-code"]'], address["zip"], 12, "邮编")
        # 关键：地址在 Stripe Address Element(iframe)里，最后填的 ZIP 不失焦→Stripe 不算 complete
        # →Update Address 点了不保存、卡表单永不出现。发 TAB 让 ZIP 失焦触发校验，再提交。
        page.tab_blur(['input[name="postalCode"]', 'input[autocomplete="postal-code"]'])
        time.sleep(2)
        log("[加卡] 提交账单地址(Update Address)")
        page.click_text(["Update Address", "Save address", "Add address", "Continue"], 8)
        time.sleep(2)
    ...
    if not page.wait_field_present(NUM, 20, "卡号框"):
        for attempt in range(2):
            log("[加卡] 卡表单没出 → 兜底第%d次:刷新页面重载 Stripe.js 再开" % (attempt + 1))
    ...
    if fill_mode == "cdp":
        return _fill_save_cdp(page, card, address, save_timeout)
    log("[加卡] 逐字段填卡")
