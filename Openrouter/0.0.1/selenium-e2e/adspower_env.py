...
#
# 文件定位：Openrouter/0.0.1/selenium-e2e/adspower_env.py
#
# 每账号【新建一个干净环境】→ 用完【删掉】，保证账号状态干净（不会因残留登录态短路全流程）。
# API 字段已在本机 AdsPower 实测通过（group/create、user/create、user/delete，code=0）。
#
# 自测：  python selenium-e2e/adspower_env.py --selftest        (建无代理环境→启→接管→停→删)
#        python selenium-e2e/adspower_env.py --selftest --proxy socks5://user:pass@host:port
# ═══════════════════════════════════════════════════════════════════════
...
def delete_env(user_id):
    ...
    try:
        ...
        log("已删环境 %s (code=%s)" % (user_id, j.get("code")))
    except Exception as e:
        log("删环境 %s 异常: %s" % (user_id, str(e)[:80]))
